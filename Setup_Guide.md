...
## เมนูสมัครสมาชิกใหม่ (Member Registration)

ระบบมีฟอร์มสมัครสมาชิกในตัว — กรอกข้อมูล → บันทึกลงชีต Members + สร้างเอกสารสัญญา (Google Doc) ให้ปริ้นเพื่อให้ลูกค้าเซ็น

### ตั้งค่าครั้งแรก (ทำครั้งเดียว)

1. ใน Apps Script เปิดหน้าโปรเจกต์ FITSTATION24
2. กดปุ่ม **+ (Add a file)** ข้าง Files → เลือก **HTML**
3. ตั้งชื่อไฟล์ว่า `MembershipForm` (ไม่ต้องใส่ .html — Apps Script เติมให้เอง)
4. เปิดไฟล์ `MembershipForm.html` ที่ผมให้ → คัดลอกทั้งหมด → วางทับ
5. กด **Save**
6. กลับมาที่ Google Sheets → refresh หน้า
7. เมนู **FITSTATION 24** → **เตรียมคอลัมน์สมัครสมาชิก (ครั้งแรก)** — ระบบจะเพิ่มคอลัมน์เสริมในชีต Members ให้อัตโนมัติ (เพศ, เลขบัตร, เบอร์, อีเมล, ที่อยู่, ฉุกเฉิน, แพ็กเกจ, ราคา, สุขภาพ, หมายเหตุ)

### การใช้งาน

1. เมนู **FITSTATION 24** → **📝 สมัครสมาชิกใหม่**
2. กรอกฟอร์ม:
   - ข้อมูลส่วนตัว: ชื่อ-นามสกุล อายุ เพศ เลขบัตรประชาชน
   - การติดต่อ: เบอร์ อีเมล LINE UID ที่อยู่ ฉุกเฉิน
   - แพ็กเกจ: สาขา + แพ็กเกจสมาชิก + PT เสริม (ถ้ามี)
   - โรคประจำตัว / หมายเหตุ
3. กด **💾 บันทึกและสร้างเอกสาร**
4. ระบบจะ:
   - สร้าง **MemberID** อัตโนมัติ (M0001, M0002, ...)
   - คำนวณ **วันหมดอายุ** จากจำนวนวันของแพ็กเกจ
   - เขียนแถวใหม่ลงชีต **Members**
   - สร้างเอกสารสัญญาเป็น **Google Doc**
5. คลิกลิงก์ **"📄 เปิดเอกสารสัญญา"** → กด **File → Print** หรือ **Ctrl+P** → ปริ้นให้ลูกค้าเซ็น

### ปรับแพ็กเกจ / ราคา

แก้ได้ใน `Code.gs` ที่ตัวแปร `MEMBERSHIP_PACKAGES` และ `PT_PACKAGES` (ราว ๆ ท้ายไฟล์)
...
แก้ราคา/จำนวนวัน/ชื่อแพ็กเกจได้ตามต้องการ จากนั้น Save → กลับมาเปิดฟอร์มใหม่จะเห็นแพ็กเกจที่อัปเดต

### ปรับข้อความในสัญญา

ข้อตกลง 8 ข้ออยู่ในฟังก์ชัน `generateMembershipContract_()` ใน `Code.gs` — แก้ array `terms` ได้
...
- **ส่งสัญญาเป็น PDF เข้า LINE/อีเมลลูกค้าอัตโนมัติ** — ต่อจากเมนูสมัครสมาชิก
- **e-Signature** — ให้ลูกค้าเซ็นบน iPad/มือถือแทนการปริ้น
